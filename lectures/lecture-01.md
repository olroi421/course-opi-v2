...
Ця лекція відкриває курс «Основи програмної інженерії» і відповідає на питання, яке зазвичай виникає в кожного, хто вже вміє писати код: чим професія інженера програмного забезпечення відрізняється від звичайного програмування і чому для великих систем одного вміння кодувати недостатньо. Матеріал побудований так, щоб рухатися від визначення до контексту: спочатку ми з'ясуємо, що саме є предметом програмної інженерії, потім простежимо, як і чому ця дисципліна виникла історично, а насамкінець розглянемо принципи, ролі та виклики, з якими стикається інженер ПЗ сьогодні. Ці засади — модульність, абстракція, розподіл відповідальності, командна структура — будуть використовуватися в усіх наступних лекціях курсу: у другій лекції ми розглянемо конкретні технології, якими ці принципи реалізуються на практиці, а в третій — життєвий цикл розробки (SDLC), що визначає, коли і в якому порядку ці принципи застосовуються в реальному проєкті.
...
timeline
    ...
    2020-ні : Хмарно-нативна розробка
             : AI-асистована розробка та агентні інструменти
             : Low-code/No-code платформи
```

Останній рядок цієї шкали часу — не менш важлива віха, ніж поява Waterfall чи Agile: поява AI-асистентів для написання коду (спершу як автодоповнення, а тепер як автономних агентів, здатних виконувати багатокрокові інженерні задачі) знову змінює те, що саме означає «вміти програмувати». Детальніше про це — у лекції 2, де розглядаються конкретні інструменти, та в лекції 3, де йдеться про місце AI в життєвому циклі розробки.
...
Варто одразу зробити застереження щодо навичок: за останній рік AI-асистенти для кодування (GitHub Copilot, Claude Code, Cursor та подібні) з інструментів автодоповнення перетворилися на агентів, здатних самостійно писати, тестувати й навіть комітити код за багатокроковими інструкціями. Це не скасовує потребу в перелічених нижче технічних стеках — навпаки, розуміння архітектури, читання чужого коду та вміння сформулювати задачу і перевірити результат стають ще важливішими, оскільки саме ці навички відрізняють інженера від людини, яка просто натискає «згенерувати».

**Розробник клієнтської частини (Frontend Developer):**
...
**Розробник серверної частини (Backend Developer):**
...
**3. Безпека**
   ...
   Орієнтиром для інженерів у цій сфері слугує **OWASP Top 10** — перелік найкритичніших ризиків вебзастосунків, який регулярно оновлює спільнота OWASP. У актуальній редакції 2025 року (остаточно затверджена на початку 2026 року) до трійки лідерів входять **Broken Access Control** (порушення контролю доступу), **Security Misconfiguration** (неправильна конфігурація безпеки) та нова категорія **Software Supply Chain Failures** (збої в ланцюжку постачання програмного забезпечення — вразливості в залежностях, інструментах збірки та процесах оновлення). Поява цієї нової категорії показова: сучасні загрози дедалі частіше стосуються не лише коду, який пише команда, а й усього ланцюжка бібліотек, пакетів та інфраструктурних інструментів, від яких цей код залежить.
...
У наступній лекції ми перейдемо від принципів до практики й розглянемо, якими саме технологіями — мовами програмування, фреймворками, базами даних, хмарними платформами — ці принципи реалізуються в реальних проєктах, а також те, як змінюється інструментарій інженера під впливом AI.
...
6. Як поява AI-асистентів для кодування змінює вимоги до навичок сучасного розробника?
